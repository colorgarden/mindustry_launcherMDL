...
namespace mindustry_launcher
{
    public class VersionManagementService
    {
        ...
        public VersionManagementService(ConfigService configService)
        {
        ...
        public List<GameInstanceInfo> GetAllInstalledInstances()
        {
            ...
            foreach (var root in _configService.GetConfig().ManagedFolders)
            {
        ...
        public void LoadVersionConfig(string instancePath)
        {
            ...
            if (File.Exists(configPath))
            {
                try
                {
                    CurrentVersionConfig = JsonSerializer.Deserialize<VersionConfig>(File.ReadAllText(configPath)) ?? new VersionConfig();
                }
                catch (Exception ex)
                {
                    Debug.WriteLine($"Failed to load version config: {ex.Message}");
                    CurrentVersionConfig = new VersionConfig();
                }
            }
            else
            {
                CurrentVersionConfig = new VersionConfig();
                CurrentVersionConfig.CustomRamMB = _configService.GetConfig().GlobalRamMB;
            }
        }
