...
public static class L
{
    ...
    // Caller must provide the Lang directory path
    public static string LangDirPath { get; set; } =
        Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "Lang");

    // Allow callers to override lang directory before LoadLanguage
    public static void Init(string langDir)
    {
        LangDirPath = langDir;
    ...
    {
        string path = Path.Combine(LangDirPath, $"{langCode}.json");

        if (!File.Exists(path))
        {
            langCode = "zh-CN";
            path = Path.Combine(LangDirPath, "zh-CN.json");
        }
